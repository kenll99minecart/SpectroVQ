...
class MGFCompressor():
    ...
    def CompressAll(self,verbose = 0,num_workers = 32,debug = False):
        ...
        tb = torchprocessor.SpectrumTorchBatchEncoder(self.mzList,self.intensityList,self.model,compounded = self.store_compounded)
    ...
    def Compress(self,verbose = 0,debug = False,CombineMetaData = True):
        ...
        for spectrumidx,spectrum in enumerate(self.mgfFile):
            ...
            if len(mzList) == self.batch_size:
                ...
                paramsList.clear()
                if self.outputOutOfRange:
                    self.ExtendedMzList.clear()
                    self.ExtendedIntensityList.clear()
        ...
        if self.store_compounded:
            if verbose >=1:
                print(f'Processing Compounded MetaData: {len(self.compounded_idx_list)} entries')
            # stored_dict = {
